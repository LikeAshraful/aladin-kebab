...
export default function MyOrders({ orders, locale = 'pl' }: MyOrdersProps) {
    ...
    const getStatusLabel = (status: string) => {
        return (t.orderStatus as any)[status] || status;
    };

    return (
        <AppLayout>
            <Head title={`${t.myOrdersTitle} — ${t.brandName}`} />

            <div className="max-w-4xl mx-auto px-4 sm:px-6 py-10 space-y-6">
                <div className="flex items-center justify-between pb-4 border-b border-neutral-800">
                    <div>
                        <h1 className="text-2xl sm:text-3xl font-black text-white">{t.myOrdersTitle}</h1>
                        <p className="text-xs text-neutral-400 mt-1">{t.myOrdersSubtitle}</p>
                    </div>
                </div>
                ...
                {orders.data.length === 0 ? (
                    <div className="py-20 text-center space-y-3 bg-neutral-900 border border-neutral-800 rounded-3xl p-8">
                        ...
                        <h3 className="text-base font-bold text-white">{t.noOrders}</h3>
                        <p className="text-xs text-neutral-400">{t.noOrdersSub}</p>
                        <Link
                            href="/"
                                ...
                                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                                    <div className="space-y-1">
                                        ...
                                        <div className="text-xs text-neutral-400 flex items-center gap-3">
                                            <span>{new Date(order.created_at).toLocaleDateString(currentLocale === 'en' ? 'en-US' : 'pl-PL')}</span>
                                            <span>•</span>
                                            <span>{order.branch?.name || t.brandName}</span>
                                        </div>
