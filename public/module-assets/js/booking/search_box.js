...
const airportMatcher = (params, data) => {
    ...
    if (params.term === undefined || data.id === undefined || data.text === undefined || data
        .text === '') {
        return data
    }
...
$(document).ready(function () {
    ...
    $('input[name="tabsA"]').change(function (e) {
        if (config.from) {
            $('#one-way-from-option, #round-trip-from-option').val(config.from).trigger('change')
        }
        if (config.to) {
            $('#one-way-to-option, #round-trip-to-option').val(config.to).trigger('change')
        }
    })

    // $('#checkLFD').on('shown.bs.modal', function () {
    ...
    fetch(airportUrl)
        ...
        .then(data => {
            ...
            // Iterate over the airports and add options to the fragment
            airports.forEach(data => {
                let opt = new Option(`${data.text}`, data.value, false, false);
                fragment.appendChild(opt);
            });
            // Append the fragment to the regular HTML select elements
            $('.one-way-from-option').append(fragment.cloneNode(true));
            $('.one-way-to-option').append(fragment.cloneNode(true));
            $('.round-trip-from-option').append(fragment.cloneNode(true));
            $('.round-trip-to-option').append(fragment.cloneNode(true));
        })
...
function oneWayTotalPassenger() {
    ...
    let total = parseInt(onewayqty.value) + parseInt(childqty.value) + parseInt(infanqty.value)
    let patType = 'Economy'
...
function roundChildInc() {
    if (totalRound < 9) {
        ...
        // document.getElementsByClassName("_child_age_").append(txtNewInputBox)
        ...
        for (var i = 0; i < elements.length; i++) {
            ...
            txtNewInputBox.innerHTML =
                `<input type="text" name="chd_age[]" class="form-control bg-light border mr-3 ch-${r_child}" id="ch-${r_child}" placeholder="age">`;
...
function oneWayChildInc() {
    ...
    if (totalOneWay < 9) {
        ...
        for (var i = 0; i < elements.length; i++) {
            let txtNewInputBox = document.createElement('div');
            txtNewInputBox.innerHTML =
                `<input type="text" name="chd_age[]" class="form-control bg-light border mr-3 ch-${o_child}" id="ch-${o_child}" placeholder="age">`;
